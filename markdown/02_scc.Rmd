...
Next, we will download the phenotype metadata using GEOquery. `getGEO` will download all the data into a `list` because some GSE entries has multiple associated data sets. GSE67097 only has one so we will extract the data using `[[1]]`.
...
if(!file.exists('data/scc/scc_rawdata.rds')){
  saveRDS(rawdata, file = 'data/scc/scc_rawdata.rds')
}
## start analysis
pheno <- pData(rawdata)
head(pheno)
